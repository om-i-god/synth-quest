	...
	alloc {
		...
		SynthDef(\sq_mage, {
			arg out=0, freq=440, amp=0.28, amp_mul=1,
			    ...
			    wet=0, extra_wet=0, delay_amt=0,
			    t_trig=0;
			var env, mod, car, sig, dry, rev, extra_rev, del;
			...
			dry = sig;
			rev = FreeVerb.ar(sig, 1, 0.88, 0.35);
			sig = (dry * (1 - wet)) + (rev * wet);
			extra_rev = FreeVerb.ar(sig, 1, 0.92, 0.4);
			sig = sig + (extra_rev * extra_wet * 1.6);
			del = CombC.ar(sig, 0.6, 0.375, 1.5);
		...
		SynthDef(\sq_cleric, {
			arg out=0, freq=220, amp=0.22, amp_mul=1,
			    ...
			    detune=0.005, wet=0, extra_wet=0, delay_amt=0,
			    t_trig=0;
			var env, sig, chorus, dry, rev, extra_rev, del;
			...
			dry = sig;
			rev = FreeVerb.ar(sig, 1, 0.92, 0.4);
			sig = (dry * (1 - wet)) + (rev * wet);
			extra_rev = FreeVerb.ar(sig, 1, 0.94, 0.45);
			sig = sig + (extra_rev * extra_wet * 1.6);
			del = CombC.ar(sig, 0.6, 0.5, 2.0);
			sig = sig + (del * delay_amt * 1.3);
			Out.ar(out, sig.dup);
		}).add;
		...
		SynthDef(\sq_warrior, {
			arg out=0, freq=110, amp=0.3, amp_mul=1,
			    ...
			    wet=0, extra_wet=0, delay_amt=0,
			    t_trig=0;
			var env, sig, sub, dry, rev, extra_rev, del;
			...
			sig = sig * env * amp * amp_mul;
			dry = sig;
			rev = FreeVerb.ar(sig, 1, 0.85, 0.5);
			sig = (dry * (1 - wet)) + (rev * wet);
			extra_rev = FreeVerb.ar(sig, 1, 0.90, 0.5);
			sig = sig + (extra_rev * extra_wet * 1.6);
			del = CombC.ar(sig, 0.6, 0.25, 1.0);
			sig = sig + (del * delay_amt * 1.3);
			Out.ar(out, sig.dup);
		}).add;
		...
		SynthDef(\sq_bard, {
			arg out=0, freq=330, amp=0.26, amp_mul=1,
			    ...
			    wet=0, extra_wet=0, delay_amt=0,
			    t_trig=0;
			var env, sig, sine, saw, dry, rev, extra_rev, del;
			env = EnvGen.kr(Env.perc(env_attack, env_release, 1, -4), t_trig);
			sine = SinOsc.ar(freq);
			...
			sig = sig * env * amp * amp_mul;
			dry = sig;
			rev = FreeVerb.ar(sig, 1, 0.9, 0.4);
			sig = (dry * (1 - wet)) + (rev * wet);
			extra_rev = FreeVerb.ar(sig, 1, 0.93, 0.45);
			sig = sig + (extra_rev * extra_wet * 1.6);
			del = CombC.ar(sig, 0.6, 0.375, 1.5);
			sig = sig + (del * delay_amt * 1.3);
			Out.ar(out, sig.dup);
